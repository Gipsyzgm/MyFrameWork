...
public class ToolsHelper
{
    ...
    public static Vector3 WorldPosToUiPos(Vector3 WorldPos, Canvas uiCanvas)
    {
        ...
        Vector2 v;
        RectTransformUtility.ScreenPointToLocalPointInRectangle(uiCanvas.transform as RectTransform, pt,
            uiCanvas.worldCamera, out v);
    ...
    public static void SetWebImage(GameObject go, string url, bool resetSize = false)
    {
        if (go != null && url != null)
        {
            UIWebImage image = go.GetComponent<UIWebImage>();
            if (image != null)
            {
                image.LoadImage(url, resetSize);
                return;
            }
        }
